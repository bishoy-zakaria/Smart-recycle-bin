...
volatile uint32 Timer0_over_flow=0;
volatile uint32  init_value=0;
volatile uint32 Timer0_Compare_Match=0;
volatile uint32 count0=0;
...
void timer0_delay(float32 delay_ms )
{
	#if Timer0_Mode     ==      TIMER_Normal_Mode
	{		
	  uint8 tick_time = Timer0_CLK /Freq;    // micro second
		
	  float32 num_tick = ((delay_ms / 2.5)*1000)/tick_time;
	...
	#elif Timer0_Mode     ==      TIMER_CTC_Mode 
	{
		 uint32 tick_time = Timer0_CLK /Freq;    // micro second
		 
		 float32 num_tick = ((delay_ms * 6 /2.5)*1000)/tick_time;
...
ISR (TIMER0_OVF_vect)
{
	...
	while (count0 == Timer0_over_flow)
	{
		...
		DIO_WritePin(DIO_PORTB , DIO_PIN7, DIO_PIN_HIGH);
		count0=0;
		timer0_stop();
	}
}
